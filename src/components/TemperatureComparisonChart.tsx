...
import {
  calculateHDDFromWeather,
  analyzeEfficiencyImprovement,
} from '../functions/temperatureNormalization';
import {
  TOTAL_TIER_1_KEY,
  TOTAL_TIER_2_KEY,
...
const getTierConsumption = (row: TieredSmartMeterRow) => {
  ...
  const t1 =
    typeof tier1 === 'string'
      ? parseFloat((tier1 as string).replace(' kWh', ''))
      : (tier1 as number);
  const t2 =
    typeof tier2 === 'string'
      ? parseFloat((tier2 as string).replace(' kWh', ''))
      : (tier2 as number);
...
const getTOUConsumption = (row: TOUSmartMeterRow) => {
  ...
  const midPeakVal =
    typeof midPeak === 'string' ? parseFloat(midPeak.replace(' kWh', '')) : midPeak;
  const offPeakVal =
    typeof offPeak === 'string' ? parseFloat(offPeak.replace(' kWh', '')) : offPeak;

  return (
    (isNaN(onPeakVal) ? 0 : onPeakVal) +
    (isNaN(midPeakVal) ? 0 : midPeakVal) +
    (isNaN(offPeakVal) ? 0 : offPeakVal)
  );
};
  ...
  createEffect(() => {
    try {
      ...
      const comparisonCost =
        baselineConsumption > 0 ? baselineCost * (comparisonConsumption / baselineConsumption) : 0;

      // Get HDD for each period
      const baselineHDD = calculateHDDFromWeather(state.weatherData, state.dateRange);
      const comparisonHDD = calculateHDDFromWeather(
        state.comparisonWeatherData,
        state.comparisonDateRange,
      );
      ...
      {!loading() && !error() && analysis() && (
        <>
          ...
          <div class="info-box">
            ...
            <p>
              <strong>Actual Consumption Change:</strong>
              <br />
              {analysis()!.actualConsumptionChange > 0 ? '↑' : '↓'}{' '}
              {Math.abs(analysis()!.actualConsumptionChange).toFixed(2)} kWh
              <br />({analysis()!.actualConsumptionChangePercent > 0 ? '+' : ''}
              {analysis()!.actualConsumptionChangePercent.toFixed(1)}%)
            </p>

            <p>
              <strong>Temperature-Adjusted Consumption:</strong>
              <br />
            ...
            <p
              style={{
                color: analysis()!.efficiencyImprovement > 0 ? '#22c55e' : '#ef4444',
                'font-weight': 'bold',
              }}
            >
              <strong>Real Efficiency Improvement:</strong>
              <br />
              {analysis()!.efficiencyImprovement > 0 ? '✓' : ''}{' '}
              {analysis()!.efficiencyImprovement.toFixed(2)} kWh
              <br />({analysis()!.efficiencyImprovementPercent > 0 ? '+' : ''}
              {analysis()!.efficiencyImprovementPercent.toFixed(1)}%)
            </p>
