...
import type { SmartMeterRow, TieredSmartMeterRow } from "../types/SmartMeter";
  ...
  Papa.parse(csvFile, {
    ...
    complete: (results: ParseResult<VueEnergyMonitorRow>) => {
      ...
      for (const row of results.data) {
        ...
        const heatPumpValue: string =
          row["Vue Energy Monitor-Heat Pump-Heat Pump (kWhs)"];
        ...
        if (!groupedByDate.has(datePart)) {
          groupedByDate.set(datePart, {
            ...
            "11 pm KWH Usage": 0,
            "12 pm KWH Usage_1": 0,
            "[touInquiry_download_Total_Tier_1_Consumption]": 0,
            "[touInquiry_download_Total_Tier_2_Consumption]": 0,
          });
        }
